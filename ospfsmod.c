...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		if (od->od_ino != 0) { //ignore empty direntries
			//grab the inode
			entry_oi = ospfs_inode(od->od_ino);
			//check its type
			switch (entry_oi->oi_ftype) {
				case OSPFS_FTYPE_REG:
					ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos-2, od->od_ino, DT_REG);
					break;
				case OSPFS_FTYPE_DIR:
					ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos-2, od->od_ino, DT_DIR);
					break;
				case OSPFS_FTYPE_SYMLINK:
					ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos-2, od->od_ino, DT_LNK);
					break;
			}
		}

		//check if filldir passed, return 0 if not
		if (ok_so_far < 0) {
			r = 0;
			break;
		}

		//if all is good, increment f_pos
